...
12. we know on a machine using docker we can have two diff containers , but if we use docker network that helps our containers to interact with each other without a port or a local host
   ...
   5. We use docker network type bridge , that is used to create a bridge kind of thing for connecting 2 or containers on same host machine.
...
Using command docker volume create VOL_Name - we can also create a custom volume , here docker itself chooses some space for our volume.
